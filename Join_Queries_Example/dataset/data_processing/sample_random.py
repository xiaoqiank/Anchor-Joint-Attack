...
def get_unique_column_values(con, table, column, year):
    """Fetches unique, non-null values for a given column and year from the database."""
    query = f'SELECT DISTINCT "{column}" FROM {table} WHERE year = {year} AND "{column}" IS NOT NULL'
    df = con.execute(query).fetchdf()
    if pd.api.types.is_string_dtype(df[column]):
        return sorted(df[column].str.strip().dropna().unique().tolist())
...
def main():
    ...
    base_output_dir = "dataset/frequency"
    ...
    for year in years:
        ...
        unique_districts = get_unique_column_values(con, table1, where_col, year)
        if not unique_districts:
            print(f"  No unique districts found in {year}. Skipping.")
            continue
        
        for rate in sample_rates:
            sample_size = int(len(unique_districts) * rate)
            sampled_districts = random.sample(unique_districts, sample_size)
